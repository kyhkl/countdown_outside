...
rt_int8_t uart_get_cnt_data(rt_int32_t time_out)
{
   rt_uint32_t e;
	 rt_err_t ret;
	 rt_uint16_t counter=0;
	 rt_uint16_t rev_valid_flg = 0,timeout_cnt=0;
	 
	
	rt_int32_t next_timeout = time_out;
	
	while(1){
			/* 接收事件 */
			ret = rt_event_recv(&rcv_event, UART_CNT_RX_EVENT|UART_CNT_RXIDLE_EVENT,RT_EVENT_FLAG_OR |
													RT_EVENT_FLAG_CLEAR,next_timeout, &e);
			if(ret == RT_EOK){
...
static void rcv_process_thread_entry(void *parameter)
{
    ...
    while (1)
    {  
				if(uart_get_cnt_data(RT_TICK_PER_SECOND/5) == 0)
